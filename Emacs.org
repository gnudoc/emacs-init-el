...
I'm using the [[https://github.com/jwiegley/use-package][use-package]] system here, rather than the built-in package.el system. Consider elpaca in future?
#+begin_src emacs-lisp
...
* Basic UI Config
Maximise the frame. Make default font sizes in case we want them. Hide away some UI elements that we don't want, like the scroll-bar, tool-bar, tooltips and menu-bar.
#+begin_src emacs-lisp
...
* Terms / Shells
** Vterm
Vterm is useful for when eshell isn't enough. Install cmake first so that vterm-module can be compiled.
#+begin_src emacs-lisp
  (use-package vterm
    :ensure t
    :config
...
* Sudo Edit
Switch to editting a file with sudo privilege after opening a file without sudo.
#+begin_src emacs-lisp
