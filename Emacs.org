...
#+begin_src emacs-lisp
  (defun nij/org-mode-setup ()
    ...
    (when (string-equal (buffer-file-name)
      ...
      (add-hook 'after-save-hook #'nij/org-babel-tangle-this-file nil t))
    )
...
#+begin_src emacs-lisp
  (defun nij/org-babel-tangle-this-file ()
    "Tangle the current org file, bypassing babel evaluation confirmation.
  This function is intended to be used as a buffer-local hook
  only in the Emacs.org file itself."
...
#+begin_src emacs-lisp
  (use-package kind-icon
    ...
    :after corfu
    ;:config
    ;; This function will add the icon formatter to corfu
    ;; *after* corfu is loaded.
    ;(kind-icon-set-corfu-format)
    )
...
- Figure out the ispell-alternate-dictionary issue - corfu spams errors in the messages buffer
