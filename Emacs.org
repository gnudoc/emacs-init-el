...
It doesn't need to run Version Control commands or git status on remote files, so let's prevent that.
#+begin_src emacs-lisp
    ...
    (add-hook 'find-file-hook
            (lambda ()
              (when (file-remote-p default-directory)
                ...
                (setq-local doom-modeline-buffer-file-name-style 'truncate-with-project)
                ;; Stop it from running 'git status' on the remote NAS
...
#+begin_src emacs-lisp
  ...
  (use-package eglot
    ...
    :config
    ;; Tell clangd to assume C23 by default if no build file is found
...
#+end_src
**** C, C++
C, C++ and cmake settings - using the newer treesitter modes for syntax, and gdb-mi for debugging
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (defun nij/c-ts-mode-common-setup ()
    ...
    ;; Smart compile command: if Makefile exists, do nothing.
    ;; So C-c c -> #'compile, which is make -k by default.
    ;; If not, and we are in a C file, just gcc it.
    (unless (file-exists-p "Makefile")
      (setq-local compile-command 
  		(concat "gcc -std=c23 -Wall -Wextra -g " 
...
#+begin_src emacs-lisp
  (use-package gdb-mi
    :ensure nil
    :config
    (setq gdb-many-windows t) ;; Use the IDE-like layout
    (setq gdb-show-main t))   ;; Start display at main()
#+end_src
**** Java
...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    :init
    (projectile-mode 1)
    ;; Disable Projectile completely for remote files
    (defun nij/disable-projectile-on-remote ()
    (when (file-remote-p default-directory)
      (projectile-mode -1)))
    (add-hook 'find-file-hook #'nij/disable-projectile-on-remote)
  ...
  (use-package dired 
    ...
    (setq dired-guess-shell-alist-user
  	'(("." ;; The catch-all for everything else
  	   (list 
  	    "mv -t /mnt/main-pool/audiobooks *" 
  	    "mv -t /mnt/main-pool/staging-audiobooks/for-auto-m4b/recentlyadded *"
  	    "rsync -av --info=progress2 --remove-source-files --append-verify --bwlimit=50m * 192.168.8.10:/mnt/tank/media/"
  	    ))))
    ;; Minimise unnecessary checks for changes in remote dired buffers
...
#+begin_src emacs-lisp
  (use-package tramp
    :ensure nil
    :config
    ;; password clears from memory every 16s. Let it persist for 10min.
    (setq password-cache-expiry 600) ; replace the number with nil for no expiry
    
    ;; The next few lines try to simplify remote connections
    (setq tramp-default-remote-shell "/bin/bash")
    (connection-local-set-profile-variables 'tramp-connection-local-default-shell-profile
                                          '((shell-file-name . "/bin/bash")
                                            (shell-command-switch . "-c")))
    (connection-local-set-profiles
     '(:application tramp :protocol "ssh") 'tramp-connection-local-default-shell-profile)
    (connection-local-set-profiles
     '(:application tramp :protocol "sudo") 'tramp-connection-local-default-shell-profile)

    ;; Don't bother with git stuff on remote files
